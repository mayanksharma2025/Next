...
const baseResolvers = {
  ...
  Mutation: {
    ...
    adminDeleteTask: async (
      _: any,
      { id }: any,
      { req }: { req: AuthRequest },
    ) => {
      if (!req.user || req.user.role !== "admin") throw new Error("Admin only");
...
export const extendedResolvers = {
  Query: {
    projects: async (
      _: any,
      { limit = 10, offset = 0, search = "", members = [], tasks = [] }: any,
      { req }: any,
    ) => {
      if (!req.user) throw new Error("Not authenticated");
      const filter: any = {};
    ...
    notifications: async (_: any, __: any, { req }: any) => {
      if (!req.user) throw new Error("Not authenticated");
      return await Notification.find({ user: req.user.id })
        .sort({ createdAt: -1 })
        .populate("user");
    },
